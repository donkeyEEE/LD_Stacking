...
class Args:
    def __init__(self, train, test, predict_df=None,
                 AFP=False, RF=False, MPNN=False, SVR=False, save_r=False,
...
def run_fun_AFP(model, train_dataset, test_dataset, epoch=5, save=False):
    train_mae = 0
    test_mae = 0
    test_R2 = 0
    recorder = []
    if True:
        train_dataset = dataloader_AFP(train_dataset)
        test_dataset = dataloader_AFP(test_dataset)
        ...
        train_mae += mean_absolute_error(y_train, train_pre)
        print('train_mae', mean_absolute_error(y_train, train_pre))

        y_val = test_dataset.y
        pre = model.predict(test_dataset)
        test_mae += mean_absolute_error(y_val, pre)
        test_R2 += r2_score(y_val, pre)
        print('test_mae', mean_absolute_error(y_val, pre))
        print('test_R2', r2_score(y_val, pre))
        # 保存fold的结果
...
def run_fun_RP(model_RF, train_dataset, test_dataset):
    train_mae = 0
    test_mae = 0
    test_R2 = 0
    recorder_RF = []
    for i in range(1):
        train_dataset = dataloader_RF_SVR(train_dataset)
        test_dataset = dataloader_RF_SVR(test_dataset)
...
def run_fun_SVR(model_SVR, train_dataset, test_dataset):
    train_mae = 0
    test_mae = 0
    test_R2 = 0
    recorder_SVR = []
